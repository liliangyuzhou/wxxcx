...
class ActivityProxy(object):
    def __init__(self, root_node, resgurad=None):
        ...
        self._ui_views = [
            UiView(node, resgurad) for node in root_node.getElementsByTagName("node")
        ]
...
class UiView(BaseView):
    ...
    def surrport_action_ids(self):
        action_ids = []
        if self.clickable:
            action_ids.append(MOTION_EVENT.CLICK)
        if self.long_clickable:
            action_ids.append(MOTION_EVENT.LONG_CLICK)
        return action_ids
    ...
    def group_name(self):
        ...
        return u", ".join(
            [
                ...
                u"class=" + self.cls_name,
                u"resource-id=" + self.rid,
                u"text=" + self.text,
                u"content-desc=" + self.content_desc,
                u"size=%d" % self.size,
            ]
        )
    ...
    def get_children(self):
        return [
            UiView(child_node)
            for child_node in self._node.childNodes
    ...
    def __eq__(self, obj):
        ...
        ret = (
            ...
            and self.cls_name == obj.cls_name
            and self.x1 == obj.x1
            and self.y1 == obj.y1
            and self.x2 == obj.x2
            and self.y2 == obj.y2
            and self.rid == obj.rid
            and self.content_desc == obj.content_desc
            and (self.text == obj.text if self.cls_name.endswith(".EditText") else True)
        )
        return ret
    ...
    def __str__(self):
        return u", ".join(
            [
                u"package=" + self.package,
                u"class=" + self.cls_name,
                u"enabled=%s" % self.enabled,
                u"resource-id=" + self.rid,
                u"text=" + self.text,
                u"content-desc=" + self.content_desc,
                u"checked=%s" % self.checked,
                u"bounds=" + self.bound_raw,
                u"display_id=%s" % self.display_id,
            ]
        )
