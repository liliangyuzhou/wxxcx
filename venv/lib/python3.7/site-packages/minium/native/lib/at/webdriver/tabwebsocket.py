...
class TabWebSocket(object):
    ...
    def close(self, timeout=CLOSE_TIMEOUT):
        "关闭当前连接，幂等，允许多次调用"
        if not WsPools.has(self.identity):
            return
    ...
    def run_script(self, script, timeout=MAX_WAIT_TIMEOUT):
        ...
        if data is None:
            raise WebSocketLoseError(
                "can not get script return, maybe webSocket crash happened"
            )
        return data
    ...
    def await_promise(self, promise_object_id, timeout=MAX_WAIT_TIMEOUT):
        "不用使用, 1.1不支持"
        data = self.sync_request(
            "Runtime", "awaitPromise", {"promiseObjectId": promise_object_id}, timeout
        )
        return data
