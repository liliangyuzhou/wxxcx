...
class QiOSNative(QBaseNative):
 ...
 def start_qq(self):
  ...
  device.find_app(bundle_id=self.bundle_id) 
  for i in range(3):
   try:
    logger.info("第 %d 次启动QQ, 共3次机会"%(i+1))
    self.app=WdaUI(server_url="http://localhost:%s"%self.wda_runner.port,bundle_id=self.bundle_id,)
 ...
 def connect_QQminiApp(self,scheme_url):
  ...
  mini_app_command=("ios-deploy  --justlaunch --bundle "+"%s/schemeAgent.app"%(current_path)+" -a "+"mqqapi://microapp/open?url=%s"%(scheme_url_urlencode))
...
if __name__=="__main__":
 ...
 mini_app_command=("ios-deploy  --justlaunch --bundle "+"./schemeAgent.app"+" -a "+"mqqapi://microapp/open?url=%s"%(scheme_url))
